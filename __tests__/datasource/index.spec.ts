...
describe('AbstractDataSource', () => {
    ...
    it('convertObjIDToIndex returns empty string if not path', () => {
        expect(dummy.convertObjIDToIndex({itemPath: ''})).resolves.toHaveProperty('jsonObjectID', '');
    ...
    it('convertObjIDToIndex convert valid hashes to idx', async () => {
        let path = await dummy.convertObjIDToIndex({ itemPath: 'arrayOfObjects/ab0000000000000000000001'});
        expect(path).toHaveProperty('jsonObjectID', 'arrayOfObjects/1');
        path = await dummy.convertObjIDToIndex({ itemPath: 'arrayOfObjects/0000000000000000000000a0'});
        expect(path).toHaveProperty('jsonObjectID', 'arrayOfObjects/0');
        path = await dummy.convertObjIDToIndex({ itemPath: 'arrayOfObjects/ab0000000000000000000001/foo/bar'});
        expect(path).toHaveProperty('jsonObjectID', 'arrayOfObjects/1/foo/bar');
        path = await dummy.convertObjIDToIndex({ itemPath: 'arrayOfObjects/1/foo/bar'});
        expect(path).toHaveProperty('jsonObjectID', 'arrayOfObjects/1/foo/bar');
        path = await dummy.convertObjIDToIndex({ itemPath: 'arrayOfObjects/ab0000000000000000000000/foo/bar'});
        expect(path).toHaveProperty('jsonObjectID', 'arrayOfObjects/ab0000000000000000000000/foo/bar');
        path = await dummy.convertObjIDToIndex({ itemPath: 'notValid/ab0000000000000000000000/foo/bar'});
